...
export class DevModeManager {
    constructor() {
        ...
        this.gridLayer = null; // Grid overlay for tile placement
        this.tileOverlays = new Map(); // Cache for tile overlay meshes
        this.selectedTile = null;
        this.popup = null;
        this.backdrop = null; // Backdrop to prevent click-through
    ...
    init(scene) {
        ...
        this.scene.add(this.visualizationLayer);
    ...
    createOverlaysBatched() {
        ...
        const walkableGeometry = new THREE.PlaneGeometry(0.9, 0.9);
        const nonWalkableGeometry = new THREE.PlaneGeometry(0.9, 0.9);
        
        // Create materials once and reuse
        const walkableMaterial = new THREE.MeshBasicMaterial({
            color: 0x00ff00,
            transparent: true,
            opacity: 0.2, // Reduced opacity for less visual interference
            side: THREE.DoubleSide,
            depthWrite: false // Prevent z-fighting issues
        });
        
        const nonWalkableMaterial = new THREE.MeshBasicMaterial({
            color: 0xff0000,
            transparent: true,
            opacity: 0.2,
            side: THREE.DoubleSide,
        ...
        // Create overlays more efficiently
        let walkableCount = 0;
        let nonWalkableCount = 0;
        
        tiles.forEach(tile => {
            if (tile.userData.walkable) walkableCount++;
        ...
        tiles.forEach(tile => {
            ...
            const geometry = isWalkable ? walkableGeometry : nonWalkableGeometry;
            const material = isWalkable ? walkableMaterial : nonWalkableMaterial;
        ...
        console.log(`Created ${this.tileOverlays.size} overlays (optimized)`);
    }
    ...
    saveTileChanges() {
        ...
        try {
            ...
            // Update the tile's visual material
            this.updateTileVisual(this.selectedTile, type);
    ...
    createNewTile(x, y, tileType = '') {
        ...
        // Update visualization to show the new tile
        this.refresh();
    ...
    createTileArea(start, end) {
        ...
        // Batch save after modifying all tiles
        this.refresh();
        this.debouncedSave();
    }
    ...
    createNewTileQuiet(x, y, tileType = '', walkable = true) {
        // Same as createNewTile but without undo tracking
        const newTileData = {
            x: x,
            y: y,
            walkable: walkable,
        ...
        if (tileType && tileType !== '') {
            try {
                ...
                material = new THREE.MeshBasicMaterial({ 
                    color: newTileData.originalColor,
                    side: THREE.DoubleSide
                });
            }
        } else {
    ...
    modifyTileQuiet(tile, newTileType, newWalkable) {
        ...
        this.updateTileVisual(tile, newTileType);
